...
/// Serialized parent for suites whose result depends on WALL-CLOCK time, plus the heavy
/// fixtures that would distort them.
///
/// `.serialized` on a suite only orders that suite's own children, not other top-level
/// suites, so a timing gate declared at top level still runs concurrently with everything
/// else. That is exactly what broke CI once: the million-file duplicate benchmark, the
/// 200k-node search benchmark and the 350k-directory snapshot fixture were saturating the
/// runner's cores while `Layout Budget Timing Gate` was measuring scan wall time, and the
/// gate failed on CI while passing locally on a machine with more cores to spare.
///
