# treemap-render-style Specification

## Purpose
Define selectable Cushion and Folders treemap rendering with honest density fallback, adaptive geometry, nested containers, and accurate interaction.
## Requirements
...
In Card style, directories SHALL be drawn as rounded containers whose children are inset within them, with a header label shown when the container is large enough to fit one legibly. Leaves SHALL be drawn flat in the selected depth colour, with the adaptive edge treatment rather than cushion lighting or whole-tile gradients.
...
### Requirement: Hit testing follows drawn geometry
Hover and selection SHALL resolve against the DRAWN rectangles - the post-nesting display rects Card style actually paints - while the fragment shader's visual gap SHALL never be subtracted from them, so card edges and sibling gaps remain clickable and suppressed content resolves upward to its nearest drawn ancestor.

#### Scenario: Selection matches the cursor
- **WHEN** the user hovers a nested child card in Card style
- **THEN** the highlighted and selectable node is the one whose drawn rect contains the cursor, with no offset between what is drawn and what responds

### Requirement: Native Folders review exposes ten real renderer schemes

Folders SHALL expose exactly ten named schemes driven by the production Swift renderer. Switching scheme SHALL repaint the current tree without rescanning,
...
- **WHEN** the user selects another scheme
...
Cushion SHALL ignore every Folders scheme input. Changing the Folders scheme SHALL not
...
Folders SHALL expose exactly six named surface profiles that are independent of the ten depth
palettes. Switching surface SHALL repaint and, where required,
