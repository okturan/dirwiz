...
- [x] 1.1 An `EphemeralPaths` type in DirWizCore, pure and injectable like `WarmStartPlanner`
...
- [x] 1.2 Handle both path-form landmines explicitly, with tests: the trailing slash `confstr`
...
- [x] 1.3 Decide and document whether `~/Library/Caches` joins the set. Argument for: same
...
- [x] 1.4 `DIRWIZ_NO_EPHEMERAL_DEFER=1` escape hatch, matching the repo's habit of shipping one
      beside every scanner behaviour change.

## 2. The cache-horizon problem, FIRST, because it is the silent-corruption risk

- [x] 2.1 Write the failing test before the fix: defer a subtree, persist the cache, warm start
...
- [x] 2.2 Implement the horizon rule: the persisted `lastEventId` must never be newer than the
...
- [x] 2.3 STOP CONDITION: if the horizon cannot be held correctly, stop and report. Shipping a
...
- [x] 2.4 Cover the interrupted case: app quits, or the patch is cancelled, mid-deferral.

## 3. Two-tier patch

- [x] 3.1 Partition the planner's target list into interactive and ephemeral roots. Keep the
      partition in DirWizCore so it is testable without the app.
- [x] 3.2 Splice the interactive roots as one batched compaction (unchanged path), publish, then
      sweep the ephemeral roots on a trailing lower-priority pass and splice again.
- [x] 3.3 The trailing pass must be cancellable, must respect the existing token-counter
...
- [x] 3.4 Represent the deferred-stale state rather than presenting it as fresh. Reuse the
      existing staleness vocabulary (`staleViewAsOf`, skipped-directory honesty) instead of
      inventing a parallel one.
- [x] 3.5 Run `invalidateAfterTreeMutation()` correctly for BOTH splices. The trailing splice
...
- [x] 4.1 THE GATE, non-negotiable: `patched-tree ≡ fresh-cold-scan` still holds once the
...
- [x] 4.2 Cold-scan totals are byte-identical to before this change. Only patch scheduling
      moved.
- [x] 4.3 Real-volume run on an idle machine (check `vm.loadavg` first; load already invalidated
...
- [x] 4.4 Gate on the interactive tier landing materially faster, and report the number. Do NOT
...
- [x] 4.5 Full suite plus a `CI=true` parity run. If `ScanSupervisionTests` fails, stash and
      confirm master fails identically before attributing it: known load-induced FSEvents flake,
      documented in CLAUDE.md.

## 5. Documentation

- [x] 5.1 CLAUDE.md, in the existing warm-start section rather than appended: the per-user
