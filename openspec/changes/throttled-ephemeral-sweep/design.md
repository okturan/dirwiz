...
`deferred-ephemeral-roots` shipped `EphemeralPaths` (OS-derived via `confstr`, fails open), `WarmPatchTargetTiers`, `WarmPatchCacheHorizon`, and a two-tier warm patch. The trailing tier runs unconditionally after every interactive splice. All the splice, compaction and invalidation machinery needed here already exists and is tested; what is missing is a decision about *when* to sweep, which is the same shape as `LiveRefreshPolicy` and `WarmStartPlanner`.
